...
async function checkAracColumns() {
  const { data, error } = await supabase.from('arac').select('*').limit(1)
  if (error) {
    console.log('Error:', error.message)
  } else if (data && data.length > 0) {
    console.log('Arac Columns:', Object.keys(data[0]))
  } else {
    console.log('Arac table is empty.')
  }
}
